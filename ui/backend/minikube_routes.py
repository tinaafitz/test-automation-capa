"""
Minikube routes — FastAPI router for Minikube/CAPI cluster management endpoints.

Endpoints moved here from app.py:
  GET  /api/capi/component-versions       (CAPI component versions)
  GET  /api/capi/cli-versions             (CLI tool versions)
  GET  /api/minikube/list-clusters         (list Minikube profiles)
  GET  /api/minikube/current-context       (current kubectl context)
  GET  /api/minikube/active-profile        (active Minikube profile)
  POST /api/minikube/verify-cluster        (verify Minikube cluster)
  POST /api/minikube/initialize-capi       (initialize CAPI/CAPA)
  POST /api/minikube/create-cluster        (create Minikube cluster)
  POST /api/minikube/delete-cluster        (delete Minikube cluster)
  POST /api/minikube/execute-command       (execute kubectl command)
"""
...
def _resolve(name: str):
    ...
    This keeps tests working: they patch ``app.jobs``, etc.,
...
async def initialize_minikube_capi(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        run_minikube_init_playbook_fn = _resolve("run_minikube_init_playbook")
        asyncio.create_task(asyncio.to_thread(
            run_minikube_init_playbook_fn,
            playbook_path,
            cluster_name,
            job_id,
            custom_capa_image,
