"""
Playbook routes module — FastAPI router for playbook execution endpoints
and supporting background-task helpers.

Endpoints moved here from app.py:
  POST   /api/ansible/run-playbook  — run_ansible_playbook_endpoint
  POST   /api/ansible/run-task      — run_ansible_task
  POST   /api/ansible/run-role      — run_ansible_role

Also contains:
  _run_playbook_in_thread()        — main playbook execution (sync, in thread)
  run_playbook_background()        — async wrapper around _run_playbook_in_thread
  run_ansible_task_background()    — runs ansible tasks in background (sync)
  run_minikube_init_playbook()     — runs minikube CAPI init playbook (sync)
"""
...
def _resolve(name: str):
    ...
    ``app.<name>`` takes effect even though the code lives here."""
...
def _run_playbook_in_thread(playbook: str, extra_vars: dict, job_id: str, description: str):
    ...
    # Resolve get_agent_stats from the app module so test patches on
    # ``app.get_agent_stats`` are honoured.
    get_agent_stats = _resolve("get_agent_stats")
...
async def run_ansible_playbook_endpoint(request: dict, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Resolve from app module so test patches on ``app.init_ai_agents`` work.
        _init_ai_agents = _resolve("init_ai_agents")
        _init_ai_agents(job_id)
