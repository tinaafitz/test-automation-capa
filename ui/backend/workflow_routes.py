"""
Workflow and trigger routes module -- FastAPI router for workflow and trigger endpoints.
...
Also contains:
  WORKFLOWS_FILE          -- path to saved_workflows.json
  _normalize_workflow()   -- migrate camelCase -> snake_case
  _load_workflows()       -- read workflows from disk
  _save_workflows()       -- persist workflows to disk
  WorkflowSave            -- Pydantic model
  TriggerCreate           -- Pydantic model
  _get_trigger_or_404()   -- helper
  _fire_and_update()      -- helper
"""
...
from trigger_service import (
    ...
    _load_trigger_state as _ts_load_trigger_state,
    _save_trigger_state as _ts_save_trigger_state,
    _send_trigger_notification,
    _update_trigger_after_run as _ts_update_trigger_after_run,
    _fire_trigger_workflow as _ts_fire_trigger_workflow,
    check_rate_limit as _ts_check_rate_limit,
    TriggerScheduler,
    _trigger_scheduler,
)

# Keep raw references for app.py re-export (app.py imports these names)
_load_trigger_state = _ts_load_trigger_state
_save_trigger_state = _ts_save_trigger_state
_update_trigger_after_run = _ts_update_trigger_after_run
_fire_trigger_workflow = _ts_fire_trigger_workflow
check_rate_limit = _ts_check_rate_limit

router = APIRouter()

_trigger_logger = logging.getLogger("trigger_routes")

# Project root (repo root, two dirs up from ui/backend/)
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))


def _resolve(name):
    """Look up *name* from the ``app`` module first (where tests apply patches),
    falling back to this module's own globals.

    This ensures that ``@patch("app._load_trigger_state", ...)`` in tests
    takes effect even though the route handlers live in this module.
    """
    app_mod = sys.modules.get("app")
    if app_mod is not None:
        val = getattr(app_mod, name, None)
        if val is not None:
            return val
    return globals()[name]
...
WORKFLOWS_FILE = os.path.join(_PROJECT_ROOT, "vars", "saved_workflows.json")


def _get_workflows_file():
    """Return WORKFLOWS_FILE, respecting patches on the ``app`` module."""
    return _resolve("WORKFLOWS_FILE")
...
async def _fire_and_update(trigger, background_tasks: BackgroundTasks):
    ...
    async def _run():
        fire_fn = _resolve("_fire_trigger_workflow")
        update_fn = _resolve("_update_trigger_after_run")
        success, run_record = await fire_fn(trigger, trigger.get("vars_override", {}))
        update_fn(trigger_id, success, run_record)
...
async def fire_trigger(trigger_id: str, background_tasks: BackgroundTasks):
    ...
    await _resolve("_fire_and_update")(trigger, background_tasks)
