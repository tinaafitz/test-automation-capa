"""
Static routes — FastAPI router for lightweight / mostly-static endpoints.
...
Endpoints moved here from app.py:
  ...
  POST /api/analyze-yaml          (YAML analysis)
  GET  /api/onboarding/tour       (onboarding tour steps)
  GET  /api/diagnostics/checks    (available diagnostics)
  POST /api/diagnostics/run       (run diagnostics)
  GET  /api/environment/overview  (environment overview)
  GET  /api/user/profile          (user profile)
  GET  /api/build/templates       (build templates)
  POST /api/validate              (validate cluster config)
"""
...
def _resolve(name: str):
    ...
    This keeps tests working: they patch ``app.get_rosa_status``, etc.,
...
# NOTE: validate_config uses ClusterConfig (Pydantic model defined in app.py).
# We cannot import it at module level due to circular imports, so the endpoint
# is registered by app.py *after* ClusterConfig is defined.  The function
# itself lives here for code-organisation purposes.

async def validate_config(config):
    """Validate cluster configuration.

    ``config`` is a ``ClusterConfig`` instance injected by FastAPI.
    The route is registered by app.py (not via the router) so that the
    Pydantic model annotation can reference the class directly.
    """
    errors = []
    warnings = []
