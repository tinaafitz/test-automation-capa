"""
AWS Dashboard routes -- FastAPI router for AWS usage, trend, config, resource details,
plus Jenkins test-results-trend and GitHub repo-activity endpoints.

Endpoints moved here from app.py:
  GET  /api/jenkins/test-results-trend     (Jenkins CI test trend data)
  GET  /api/github/repo-activity           (GitHub repository activity)
  GET  /api/aws/usage                      (AWS resource usage counts)
  GET  /api/aws/usage-trend                (AWS usage trend over time)
  GET  /api/aws/usage-config               (AWS resource config with quotas)
  GET  /api/aws/usage/{resource_key}       (single resource count refresh)
  GET  /api/aws/resource-details/{resource_type}  (detailed resource info)

Also contains:
  _collect_aws_usage_data()        -- sync helper to gather AWS resource counts
  _get_aws_history_db()            -- path to SQLite history DB
  _init_aws_history_db()           -- create history table if needed
  _save_aws_usage_snapshot()       -- persist a snapshot to DB
  _aws_usage_snapshot_loop()       -- background coroutine (hourly snapshots)
"""
...
def _resolve(name: str):
    ...
    This ensures that ``@patch("app.<name>", ...)`` in tests takes effect even
    though the route handlers live in this module.
    """
...
# ---------------------------------------------------------------------------
# _collect_aws_usage_data  (sync helper)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Background snapshot loop (called from app.py startup event)
# ---------------------------------------------------------------------------
