"""
Cluster-actions routes — FastAPI router for the Feature Action Engine + ClusterAutomationSpec APIs.

Endpoints moved here from app.py:

Cluster-action endpoints:
  GET  /api/cluster-actions/features                    (feature registry)
  GET  /api/cluster-actions/features/{suite_id}         (suite features)
  POST /api/cluster-actions/execute                     (execute feature actions)
  GET  /api/cluster-actions/history                     (action history)
  POST /api/cluster-actions/provision                   (provision with features)
  GET  /api/cluster-actions/discover                    (discover clusters)
  GET  /api/cluster-actions/cluster/{cluster_name}/status (cluster feature status)

ClusterAutomationSpec endpoints:
  GET  /api/cluster-specs                               (list specs)
  GET  /api/cluster-specs/{spec_id}                     (get spec)
  POST /api/cluster-specs/plan                          (plan spec)
  POST /api/cluster-specs/execute                       (execute spec)
  POST /api/cluster-specs/save                          (save spec)

Also contains:
  Feature registry wrappers (_load_feature_registry_full, _load_feature_registry, etc.)
  ClusterActionRequest   -- Pydantic model
  Action history helpers  (_load_action_history, _save_action_history, _record_action, ACTION_HISTORY_FILE)
  Per-cluster lock helpers (_get_cluster_lock, _cluster_locks, _CLUSTER_LOCKS_MAX)
  CLUSTER_FEATURE_REGISTRY, _FEATURE_INDEX -- backward-compat module-level variables
  SPECS_DIR, _find_spec_file, _resolve_spec_to_plan
"""
...
async def _execute_cluster_actions_locked(request: ClusterActionRequest):
    ...
    _find_feature_fn = _resolve("_find_feature")
    _record_action_fn = _resolve("_record_action")
    ...
    for action in request.actions:
        ...
        feature = _find_feature_fn(feature_id)
        ...
        if feature.get("playbook"):
            ...
            try:
                ...
                _record_action_fn(request.cluster_name, request.namespace, feature_id,
