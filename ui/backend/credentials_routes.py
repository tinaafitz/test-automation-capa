"""
Credentials routes -- FastAPI router for credential & status-check endpoints.

Endpoints moved here from app.py:
  GET  /api/config/status           (check user_vars.yml configuration)
  GET  /api/credentials             (read credentials from user_vars.yml)
  POST /api/credentials             (save credentials to user_vars.yml)
  GET  /api/rosa/status             (ROSA CLI authentication status)
  GET  /api/ocp/connection-status   (OpenShift Hub connection test)
  GET  /api/aws/credentials-status  (AWS credential validation)
  GET  /api/guided-setup/status     (comprehensive guided setup status)
"""
